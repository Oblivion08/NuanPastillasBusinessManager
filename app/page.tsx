...
export default function Home() {
  ...
  const [modal, setModal] = useState<"sale" | "expense" | "product" | "stock" | "edit" | null>(null);
  ...
  async function submit(kind: string, payload: Record<string, FormDataEntryValue>) {
    try {
      ...
      setModal(null); setEditingProduct(null); setToast("Saved! Updated na ang dashboard."); await load();
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {loading ? <div className="loading">Loading your business records…</div> : <Content tab={tab} data={data} stats={stats} open={setModal} onEdit={p => { setEditingProduct(p); setModal("edit"); }} save={submit} />}
      </main>
      ...
      {modal && <EntryModal type={modal} products={data.products} product={editingProduct} defaultLowStock={data.settings.defaultLowStock} close={() => { setModal(null); setEditingProduct(null); }} submit={submit} />}
      {toast && <div className="toast">{toast}</div>}
    </div>
  );
}

function Content({ tab, data, stats, open, onEdit, save }: { tab: Tab; data: Data; stats: Stats; open: (x: "sale" | "expense" | "product" | "stock") => void; onEdit: (p: Product) => void; save: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {
  if (tab === "Dashboard") return <Dashboard data={data} stats={stats} open={open} />;
  if (tab === "Sales") return <ListPage title="Sales history" action="Add Sale" onAction={() => open("sale")}><SalesTable sales={data.sales} /></ListPage>;
...
function SalesTable({ sales }: { sales: Sale[] }) { return <div className="table-wrap"><table><thead><tr><th>Date</th><th>Product</th><th>Qty</th><th>Sales</th><th>Profit</th><th>Payment</th></tr></thead><tbody>{sales.length ? sales.map(s => <tr key={s.id}><td>{s.saleDate}</td><td><b>{s.productName}</b><small>{s.channel}</small></td><td>{s.quantity}</td><td>{money.format(s.total)}</td><td className="positive">{money.format(s.profit)}</td><td><span className="tag">{s.payment}</span></td></tr>) : <tr><td colSpan={6} className="empty">No sales yet. Add your first transaction.</td></tr>}</tbody></table></div>; }
...
function EntryModal({ type, products, product, defaultLowStock, close, submit }: { type: "sale" | "expense" | "product" | "stock" | "edit"; products: Product[]; product: Product | null; defaultLowStock: number; close: () => void; submit: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {
  const labels = { sale: "Record a Sale", expense: "Add an Expense", product: "Add a Product", stock: "Update Inventory", edit: "Edit Product" };

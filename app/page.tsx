...
export default function Home() {
  ...
  const [modal, setModal] = useState<"sale" | "editSale" | "expense" | "editExpense" | "product" | "stock" | "edit" | null>(null);
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {loading ? <div className="loading">Loading your business records…</div> : <Content tab={tab} data={data} stats={stats} open={setModal} onEdit={p => { setEditingProduct(p); setModal("edit"); }} onEditSale={s => { setEditingSale(s); setModal("editSale"); }} onEditExpense={e => { setEditingExpense(e); setModal("editExpense"); }} onStockProduct={p => { setEditingProduct(p); setModal("stock"); }} save={submit} />}
      </main>
...
function Content({ tab, data, stats, open, onEdit, onEditSale, onEditExpense, onStockProduct, save }: { tab: Tab; data: Data; stats: Stats; open: (x: "sale" | "expense" | "product" | "stock") => void; onEdit: (p: Product) => void; onEditSale: (s: Sale) => void; onEditExpense: (e: Expense) => void; onStockProduct: (p: Product) => void; save: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {
...
function EntryModal({ type, products, product, sale, expense, defaultLowStock, close, submit }: { type: "sale" | "editSale" | "expense" | "editExpense" | "product" | "stock" | "edit"; products: Product[]; product: Product | null; sale: Sale | null; expense: Expense | null; defaultLowStock: number; close: () => void; submit: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {
  const labels = { sale: "Record a Sale", editSale: "Edit Sale", expense: "Add an Expense", editExpense: "Edit Expense", product: "Add a Product", stock: "Update Inventory", edit: "Edit Product" };

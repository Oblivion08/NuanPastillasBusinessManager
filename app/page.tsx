...
export default function Home() {
  ...
  async function deleteSale(sale: Sale) {
    ...
    try {
      await runTransaction(db,async tx=>{const saleRef=doc(db,"sales",String(sale.id)),productRef=doc(db,"products",String(sale.productId));const [saleSnap,productSnap]=await Promise.all([tx.get(saleRef),tx.get(productRef)]);if(!saleSnap.exists())throw new Error("Sale no longer exists.");if(!productSnap.exists())throw new Error("Product not found; inventory cannot be restored.");const savedSale=saleSnap.data() as Sale,product=productSnap.data() as Product;tx.update(productRef,{stock:product.stock+savedSale.quantity});tx.delete(saleRef);});
      setToast("Sale deleted. Quantity returned to inventory."); await load();
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {loading ? <div className="loading">Loading your business records…</div> : <Content tab={tab} data={data} stats={stats} open={setModal} onEdit={p => { setEditingProduct(p); setModal("edit"); }} onEditSale={s => { setEditingSale(s); setModal("editSale"); }} onDeleteSale={deleteSale} onEditExpense={e => { setEditingExpense(e); setModal("editExpense"); }} onStockProduct={p => { setEditingProduct(p); setModal("stock"); }} onPayment={s => { setEditingSale(s); setModal("receivablePayment"); }} save={submit} />}
      </main>
...
function Content({ tab, data, stats, open, onEdit, onEditSale, onDeleteSale, onEditExpense, onStockProduct, onPayment, save }: { tab: Tab; data: Data; stats: Stats; open: (x: "sale" | "expense" | "product" | "stock") => void; onEdit: (p: Product) => void; onEditSale: (s: Sale) => void; onDeleteSale: (s: Sale) => Promise<void>; onEditExpense: (e: Expense) => void; onStockProduct: (p: Product) => void; onPayment: (s: Sale) => void; save: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {

...
type Sale = { id: number; saleDate: string; productId: number; productName: string; quantity: number; unitPrice: number; unitCost: number; discount: number; total: number; profit: number; payment: string; channel: string; customerName?: string; dueDate?: string; amountPaid?: number; paymentStatus?: "unpaid" | "partial" | "paid" };
    ...
    try {
      ...
      if (kind === "sale") {
        ...
        await runTransaction(db,async tx=>{
          ...
          tx.set(saleRef,{id:saleId,saleDate:value.date,productId,productName:newProduct.name,quantity:qty,unitPrice,unitCost,discount,total,profit:total-qty*unitCost,payment:value.payment,channel:value.channel,customerName:isCredit?value.customerName:"",dueDate:isCredit?value.dueDate:"",amountPaid,paymentStatus});
        });
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {loading ? <div className="loading">Loading your business records…</div> : <Content tab={tab} data={data} stats={stats} open={setModal} onEdit={p => { setEditingProduct(p); setModal("edit"); }} onEditSale={s => { setEditingSale(s); setModal("editSale"); }} onEditExpense={e => { setEditingExpense(e); setModal("editExpense"); }} onStockProduct={p => { setEditingProduct(p); setModal("stock"); }} onPayment={s => { setEditingSale(s); setModal("receivablePayment"); }} save={submit} />}
      </main>
...
function Content({ tab, data, stats, open, onEdit, onEditSale, onEditExpense, onStockProduct, onPayment, save }: { tab: Tab; data: Data; stats: Stats; open: (x: "sale" | "expense" | "product" | "stock") => void; onEdit: (p: Product) => void; onEditSale: (s: Sale) => void; onEditExpense: (e: Expense) => void; onStockProduct: (p: Product) => void; onPayment: (s: Sale) => void; save: (kind: string, p: Record<string, FormDataEntryValue>) => Promise<void> }) {
  ...
  if (tab === "Receivables") return <Receivables sales={data.sales} onPayment={onPayment} />;
  if (tab === "Expenses") return <ListPage title="Expense history" action="Add Expense" onAction={() => open("expense")}><ExpenseTable expenses={data.expenses} onEdit={onEditExpense} /></ListPage>;
...
function SalesTable({ sales, onEdit }: { sales: Sale[]; onEdit?: (s: Sale) => void }) { return <div className="table-wrap"><table><thead><tr><th>Date</th><th>Product</th><th>Qty</th><th>Sales</th><th>Profit</th><th>Payment</th>{onEdit?<th>Action</th>:null}</tr></thead><tbody>{sales.length ? sales.map(s => <tr key={s.id}><td>{s.saleDate}</td><td><b>{s.productName}</b><small>{s.channel}</small></td><td>{s.quantity}</td><td>{money.format(s.total)}</td><td className="positive">{money.format(s.profit)}</td><td><span className="tag">{s.payment}</span>{s.payment==="Credit"?<small>{s.paymentStatus||"unpaid"}</small>:null}</td>{onEdit?<td><button className="edit-button" onClick={()=>onEdit(s)}>✎ Edit</button></td>:null}</tr>) : <tr><td colSpan={onEdit?7:6} className="empty">No sales yet. Add your first transaction.</td></tr>}</tbody></table></div>; }
function Receivables({ sales, onPayment }: { sales: Sale[]; onPayment: (s: Sale) => void }) { const rows=sales.filter(s=>s.payment==="Credit"),outstanding=rows.reduce((n,s)=>n+Math.max(0,s.total-(s.amountPaid||0)),0);return <section className="panel list-page"><div className="list-head"><div><h2>Accounts receivable</h2><p>Credit sales that still need to be collected</p></div><div className="receivable-total"><small>Total outstanding</small><strong>{money.format(outstanding)}</strong></div></div><div className="table-wrap"><table><thead><tr><th>Customer</th><th>Sale</th><th>Due date</th><th>Total</th><th>Paid</th><th>Balance</th><th>Status</th><th>Action</th></tr></thead><tbody>{rows.length?rows.map(s=>{const paid=s.amountPaid||0,balance=Math.max(0,s.total-paid),overdue=balance>0&&Boolean(s.dueDate)&&s.dueDate!<today();return <tr key={s.id}><td><b>{s.customerName||"Unnamed customer"}</b><small>{s.saleDate}</small></td><td>{s.productName}<small>{s.quantity} pack(s)</small></td><td>{s.dueDate||"—"}</td><td>{money.format(s.total)}</td><td className="positive">{money.format(paid)}</td><td className={balance>0?"negative":"positive"}><b>{money.format(balance)}</b></td><td><span className={`status ${balance===0?"ok":overdue?"danger":"pending"}`}>{balance===0?"Paid":overdue?"Overdue":paid>0?"Partial":"Unpaid"}</span></td><td>{balance>0?<button className="edit-button" onClick={()=>onPayment(s)}>＋ Payment</button>:"—"}</td></tr>}):<tr><td colSpan={8} className="empty">No credit sales yet.</td></tr>}</tbody></table></div></section>; }
function ExpenseTable({ expenses, onEdit }: { expenses: Expense[]; onEdit: (e: Expense) => void }) { return <div className="table-wrap"><table><thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Payment</th><th>Action</th></tr></thead><tbody>{expenses.length ? expenses.map(e => <tr key={e.id}><td>{e.expenseDate}</td><td><span className="tag">{e.category}</span></td><td>{e.description}</td><td className="negative">{money.format(e.amount)}</td><td>{e.payment}</td><td><button className="edit-button" onClick={()=>onEdit(e)}>✎ Edit</button></td></tr>) : <tr><td colSpan={6} className="empty">No expenses recorded yet.</td></tr>}</tbody></table></div>; }

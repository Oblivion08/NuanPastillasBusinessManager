...
const productImage = (flavor: string) => flavor.toLowerCase().includes("ube") ? "/nuan-ube-pastillas.webp" : "/nuan-milk-pastillas.webp";
...
export default function Home() {
  ...
  if (!user) return <div className="login-screen"><form className="login-card" onSubmit={login}><Image className="brand-logo login-logo" src="/nuan-logo.webp" alt="NUAN Pastillas logo" width={120} height={120} priority/><p className="eyebrow">NUAN PASTILLAS</p><h1>Business Manager</h1><p>Sign in to view your private sales, inventory, expenses, and profit records.</p><label>Email<input type="email" name="email" required autoComplete="email"/></label><label>Password<input type="password" name="password" required autoComplete="current-password"/></label><button className="primary" disabled={loading}>{loading?"Signing in…":"Sign In"}</button></form>{toast&&<div className="toast">{toast}</div>}</div>;

  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><Image className="brand-logo" src="/nuan-logo.webp" alt="NUAN Pastillas logo" width={48} height={48} priority/><div><strong>NUAN</strong><span>{data.settings.businessName}</span></div></div>
...
function Dashboard({ data, stats, open }: { data: Data; stats: Stats; open: (x: "sale" | "expense" | "product" | "stock") => void }) {
  ...
  return <>
    ...
    <section className="mini-grid"><article><span>Today’s sales</span><strong>{money.format(stats.sales)}</strong></article><article><span>Today’s expenses</span><strong>{money.format(stats.expenses)}</strong></article><article className={stats.net<0?"warning":""}><span>Today’s net income</span><strong>{money.format(stats.net)}</strong></article><article><span>Packs sold today</span><strong>{stats.packs}</strong></article></section>
    <section className="product-spotlight panel"><div className="spotlight-copy"><p className="eyebrow">NUAN PRODUCT LINE</p><h2>Our signature pastillas</h2><span>Actual products currently tracked in your business inventory.</span><div className="spotlight-badges"><b>Milk Pastillas</b><b>Ube Pastillas</b></div></div><div className="spotlight-products"><figure><Image src="/nuan-milk-pastillas.webp" alt="Actual NUAN Milk Pastillas pack" width={210} height={280} priority/><figcaption>Milk</figcaption></figure><figure><Image src="/nuan-ube-pastillas.webp" alt="Actual NUAN Ube Pastillas pack" width={210} height={280} priority/><figcaption>Ube</figcaption></figure></div></section>
    <section className="monthly-goal panel"><div className="goal-copy"><p className="eyebrow">MONTHLY PACK GOAL</p><h2>{monthlyPacks} of {goal} packs sold</h2><span>{remaining ? `${remaining} packs remaining this month` : "Goal achieved—great work!"}</span><div className="goal-numbers"><div><small>Sold</small><b>{monthlyPacks}</b></div><div><small>Goal</small><b>{goal}</b></div><div><small>Remaining</small><b>{remaining}</b></div></div></div><div className="goal-ring" style={{background:`conic-gradient(#a87330 ${goalPercent*3.6}deg,#e5d8c6 0deg)`}}><div><strong>{Math.round(monthlyPacks/goal*100)}%</strong><small>complete</small></div></div></section>
...
function ProductGrid({ products, onEdit }: { products: Product[]; onEdit: (p: Product) => void }) { return <div className="product-grid">{products.map(p => <article key={p.id}><div className={`product-art ${p.flavor.toLowerCase().includes("ube") ? "ube" : "milk"}`}><Image src={productImage(p.flavor)} alt={`Actual ${p.name} pack`} width={220} height={280}/><span>{p.flavor}</span></div><div className="product-title"><div><h3>{p.name}</h3><p>{p.flavor} • Pastillas</p></div><button className="edit-button" onClick={() => onEdit(p)}>✎ Edit</button></div><div><span>Cost <b>{money.format(p.cost)}</b></span><span>Price <b>{money.format(p.price)}</b></span><span>Profit <b className="positive">{money.format(p.price - p.cost)}</b></span></div></article>)}</div>; }
